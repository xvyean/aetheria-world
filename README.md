...
- **Blender 程序化建模**：整座空岛由 `blender/academy/build.py` 用 Python 生成——约 8,900 个部件、35.7 万顶点，全部按设定书（`docs/02-星槎志.md`）落位：
  500 × 360 米倒锥岛体（长宽均超过旧版 5 倍）、外城墙与四门、中央旧学宫、星语学宫、晨辉演武院、锤音工造院、海心港区、七年舍街、百工与疗愈区、星植苑，以及原有四院四塔、星穗馆、烬园、栈桥、渡船与陪岛石。
- **导出**：合并为 454 个网格，顶点色着色（无贴图），Draco 压缩 glTF 约 **3.7 MB**；可动部件带 `extras`（fx 类型 / 轨道参数 / 志书键），网页据此点火、转晶、涨潮、开船、摆旗
- **网页观览器**（`js/academy3d.js`）：着色器天空与云海、裂隙光柱与上升粒子、晨 / 午 / 夜三档氛围、渡船 90 秒潮汐往返、陪岛石公转、30 处**热点**——悬停高亮、点击飞入并翻开志书卡片、地名浮标自动避让与遮挡剔除、全屏、键盘翻页
- **志书**：船票、槎埠时刻、摆渡人、四院（院长 / 院规 / 塔）、分选记录、星槎七律、十四贡、仪典、建模札记
- 同一个 GLB 也被放进世界地图的裂隙上空（缩放 0.112），点击「星槎学院」即可飞赴
...
│   ├── districts.py           # 学院城外墙 / 四门 / 七个扩建校区 / 城市林带
│   └── util.py                # 网格工具、材质、顶点色、调色板
├── models/
│   ├── xingcha_academy.glb    # Draco 压缩 glTF（约 3.7 MB）
│   ├── xingcha_academy.hotspots.json   # 30 处热点（名称 / 位置 / 相机机位）
